...
login_status=0
app= Flask(__name__)
app.secret_key = 'NIRAMAYA Authentication'
...
def insure():
    ...
    found = mycol.find()
    arr = list(found)
    doctors = mydb.Nodes.find()  # Retrieve doctor data from the database
    return render_template('sd.html', doctors=doctors)

@app.route('/searchpatbyname', methods=['GET' , 'POST'])
...
def searchpatbyname():
    if request.method == 'POST':
        ...
        found = mycol.find(query)
        arr = list(found)
        
        return render_template("searchpatbyname.html", patbyname=arr)
    return render_template("sp.html")  # Assuming you have a template for the search form
...
def sp():
    ...
    found = mycol.find()
    arr = list(found)
    pati = mydb.Blockhead.find()  # Retrieve doctor data from the database
    return render_template('sp.html', pati=pati)
...
def patientverify():
    ...
    aadhar = request.form['aadhar']  # Assuming 'aadhar' is the name of the Aadhaar field in the form
    pwd = request.form['pwd']

    patquery = { "_id": userid, "aadhar": aadhar }  # Modify the query to include Aadhaar
...
def search():
    if request.method == 'POST':
        ...
        mycol = mydb['Nodes']
        # mypat = mydb['Blockhead']

        query = {}
        # pqwery = {}

        if special:
            query['specialization'] = special
        if add:
            query['address'] = add
        if name:
            query['docname'] = name
        # if patname:
        #     pqwery['first'] = patname
        
        found = mycol.find(query)
        arr = list(found)
        
        return render_template("show.html", posts=arr, special=special, add=add)

    return render_template("search.html")  # Assuming you have a template for the search form
...
#     # Retrieve user's date from the database or session
#     user_date = get_user_date()  # Implement this function
    
#     if user_date == specified_date:
#         mycol = mydb[owner]
#         records = query_records_by_date(owner, specified_date)
#         return render_template('display_by_date.html', records=records)
    
#     return redirect(url_for('back'))  # Redirect to appropriate route

# def query_records_by_date(owner, date):
#     mycol = mydb[owner]
#     records = mycol.find({'date': date})
#     return records
